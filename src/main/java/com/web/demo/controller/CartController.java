...
import com.web.demo.dto.CartDTO;
...
import com.web.demo.service.BillDetailService;
import com.web.demo.service.BillService;
...
public class CartController {
	...
	public String  addcart(Model model,HttpSession session,@PathVariable int id,HttpServletRequest req) {
		...
		HashMap<Integer, CartDTO>Cartitems=(HashMap<Integer, CartDTO>)session.getAttribute("mycartitem");
		if(Cartitems==null) {
			Cartitems=new HashMap<Integer, CartDTO>();
			
		}
		Optional<Games> games=gameservice.findById(id);
		if(games.isPresent()) {
			CartDTO cart=new CartDTO(games.get());
	...
	public double totalPrice(HashMap<Integer, CartDTO> cartitems) {
		int count=0;
